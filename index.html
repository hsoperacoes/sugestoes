# `index.html` — Portal de Sugestões / Reclamações (HS)
...
```javascript
/*************************************************
 * HS • Portal de Sugestões & Reclamações
 * Planilhas usadas:
 *  - CONFIG    => [A]Código  [B]Razão Social  [C]Senha  [D..] Funcionários
 *  - REGISTROS => Timestamp, Código, Razão, Anônimo, Funcionário, Categoria, Subtipo, Item, Cor, Tamanho, Assunto, Mensagem
 *************************************************/
...
function doGet(){
  ...
  return HtmlService
    ...
    .setTitle('HS • Portal de Sugestões & Reclamações');
}
...
function ensureSheets_(){
  ...
  let cfg = ss.getSheetByName(SH_CONFIG); if(!cfg){
    cfg = ss.insertSheet(SH_CONFIG);
    cfg.getRange(1,1,1,6).setValues([[ 'Código','Razão Social','Senha','Funcionário 1','Funcionário 2','Funcionário 3' ]]);
    // Semeia filiais padrão (ajuste os códigos conforme seu arquivo)
    const seed = [
      ['293','ARTUR MALHAS','293@hs',''],
      ['488','FLORIANO JEANS E ACESSORIOS','488@hs',''],
      ['287','JOTA MALHAS','287@hs',''],
      ['761','MODA BÁSICA','761@hs',''],
      ['288','PONTO DA MALHA','288@hs','']
    ];
    cfg.getRange(2,1,seed.length,4).setValues(seed);
    cfg.autoResizeColumns(1,6);
  }
  let reg = ss.getSheetByName(SH_REG); if(!reg){
    reg = ss.insertSheet(SH_REG);
    reg.getRange(1,1,1,12).setValues([[
      'Timestamp','Código','Razão','Anônimo','Funcionário','Categoria','Subtipo','Item','Cor','Tamanho','Assunto','Mensagem'
    ]]);
    reg.autoResizeColumns(1,12);
  }
}

function _cfgSheet_(){ ensureSheets_(); return SpreadsheetApp.getActive().getSheetByName(SH_CONFIG) }
function _regSheet_(){ ensureSheets_(); return SpreadsheetApp.getActive().getSheetByName(SH_REG) }
...
function getFiliais(){
  const sh=_cfgSheet_();
  const values = sh.getRange(2,1,Math.max(sh.getLastRow()-1,0),2).getValues().filter(r=>r[0])
  return { filiais: values.map(r=>({ codigo:String(r[0]), razao:String(r[1]) })) }
}

function getFuncionarios(codigo){
  const sh=_cfgSheet_(); const data = sh.getRange(2,1,Math.max(sh.getLastRow()-1,0),20).getValues()
  const row = data.find(r=> String(r[0])===String(codigo) );
  if(!row) return [];
  const list = row.slice(3).map(v=>String(v||'').trim()).filter(Boolean);
  return list;
}

function submitRegistro(payload){
  const sh=_regSheet_();
  const ts = new Date();
  const anon = !payload.ident;
  const row = [
    ts,
    payload.codigo||'',
    payload.razao||'',
...
/*******************
 * API — Gerência (protegido por senha por filial)
 *******************/
function authManager(codigo, senha){
  const sh=_cfgSheet_();
  const rng = sh.getDataRange().getValues(); // inclui header
  const hit = rng.slice(1).find(r=> String(r[0])===String(codigo) );
  if(!hit) throw new Error('Filial não encontrada');
  if(String(hit[2])!==String(senha)) throw new Error('Senha inválida');
  const funcionarios = hit.slice(3).map(v=>String(v||'').trim()).filter(Boolean)
  return { codigo:String(hit[0]), razao:String(hit[1]), funcionarios };
}

function updateFuncionarios(codigo, senha, lista, novaSenha){
  const sh=_cfgSheet_();
  const last = sh.getLastRow();
  const data = sh.getRange(1,1,last, sh.getLastColumn()).getValues();
  const idx = data.findIndex((r,i)=> i>0 && String(r[0])===String(codigo));
  if(idx<0) throw new Error('Filial não encontrada');
  if(String(data[idx][2])!==String(senha)) throw new Error('Senha inválida');

  // Grava funcionários a partir da coluna D
  const rowIndex = idx+1; // +1 pq data inclui header
  const baseCol = 4; // D
  const maxCols = Math.max( (sh.getLastColumn()-baseCol+1), lista.length );
  if(maxCols>0){
    // Limpa linha antiga
    sh.getRange(rowIndex+1, baseCol, 1, sh.getLastColumn()-baseCol+1).clearContent();
    // Escreve nova lista
    if(lista && lista.length){
      sh.getRange(rowIndex+1, baseCol, 1, lista.length).setValues([lista])
    }
  }

  // Atualiza senha se informado
  if(novaSenha){ sh.getRange(rowIndex+1, 3).setValue(novaSenha) }
  return true;
}

/*******************
 * Utilitário — importar HTML
 *******************/
